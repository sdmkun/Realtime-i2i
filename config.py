...
from i2i import get_pipe, LCM_run
...
class ConfigWindow:
    def __init__(self):
        ...
        master.geometry("400x600")  # ウィンドウサイズを設定
        ...
        self.prompt = tk.StringVar(value="1girl, cowboy shot, a girl with glasses, detailed face")
        ...
        self.strength = tk.StringVar(value=0.5)
        ...
        # カメラID
        ttk.Label(master, text="カメラID").grid(row=5, column=0, padx=10, pady=10, sticky="w")
        self.camera_id = tk.StringVar(value=0)
        ttk.Entry(master, textvariable=self.camera_id, width=30).grid(row=5, column=1, padx=10, pady=10)
        ...
        capture_button.grid(row=7, column=0, columnspan=2, padx=10, pady=10, sticky="ew")

    def update_param(self):
        self.num_inference_steps_value = int(self.num_inference_steps.get())
        self.strength_value = float(self.strength.get())
        self.camera_id_value = int(self.camera_id.get())
    ...
    def capture_screen(self):
        ...
        self.update_param()
        self.screen_capture = ScreenCapture(self.camera_id_value)
        pipe = get_pipe(self)
